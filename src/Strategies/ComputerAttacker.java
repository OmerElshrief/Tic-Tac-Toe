...
public class ComputerAttacker implements Player
{
Observer observer;
    private Game game;
    private panel[] p1;
    private static ComputerAttacker computer;
   

    private ComputerAttacker()
    { observer = WinObserver.GeInstance();
        game = Game.getInstance();
        p1 = game.getP1();
    }
    
    public static ComputerAttacker GetInstance()
    {
        if(computer == null)
            computer = new ComputerAttacker();
        return computer;
    }
    ...
    public void Play(panel panel) 
    {
       ...
       int panelIndex = 0;
        ...
        {
            Logger.getLogger(ComputerAttacker.class.getName()).log(Level.SEVERE, null, ex);
        }
